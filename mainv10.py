...
def autonom_navigasjon():
    ...
    maks_rotasjon = 30  # maks tillatt rotasjonsvinkel i én kommando

    forsøk = 0  # teller forsøk på å snu
    ...
    while autonom_aktiv:
        ...
        # Blokkert foran og på begge sider
        if mid < rotasjon_grense and left < rotasjon_grense and right < rotasjon_grense:
            status = "Full stopp – alt blokkert"
            send_to_arduino("MOV:X=0,Y=0,R=180")
            time.sleep(2)
            forsøk += 1

        # Blokkert på begge sider, men åpen rett frem
        elif left < rotasjon_grense and right < rotasjon_grense:
            status = "Roterer 180° – begge sider blokkert"
            send_to_arduino("MOV:X=0,Y=0,R=180")
            time.sleep(2)
            forsøk += 1

        # Hindring venstre – roter høyre
        elif left < unnam_grense and right > left + 10:
            rotasjonsgrad = min(maks_rotasjon, int((unnam_grense - left) * 0.5))
            status = f"Roterer høyre – venstre blokkert ({rotasjonsgrad}°)"
            send_to_arduino(f"MOV:X=0,Y=0,R={rotasjonsgrad}")
            time.sleep(0.4)

        # Hindring høyre – roter venstre
        elif right < unnam_grense and left > right + 10:
            rotasjonsgrad = min(maks_rotasjon, int((unnam_grense - right) * 0.5))
            status = f"Roterer venstre – høyre blokkert ({rotasjonsgrad}°)"
            send_to_arduino(f"MOV:X=0,Y=0,R=-{rotasjonsgrad}")
            time.sleep(0.4)

        # Klar bane foran
        elif mid > unnam_grense:
            fart = min(3, int(mid * 0.05))  # maks fart 3
            status = f"Fremover – hastighet {fart}"
            send_to_arduino(f"MOV:X={fart},Y=0,R=0")
            forsøk = 0

        # Sving bort fra skrå vegg mot høyre
        elif (left - mid) > 25 and mid < unnam_grense:
            status = "Skrå vegg venstre – svinger høyre"
            send_to_arduino("MOV:X=0,Y=0,R=10")
            time.sleep(0.4)
        
        # Sving bort fra skrå vegg mot venstre
        elif (right - mid) > 25 and mid < unnam_grense:
            status = "Skrå vegg høyre – svinger venstre"
            send_to_arduino("MOV:X=0,Y=0,R=-10")
            time.sleep(0.4)
      
        else:
            status = "Stopp – usikker situasjon"
            send_to_arduino("MOV:X=0,Y=0,R=0")
            forsøk += 1

        # Bare oppdater status dersom den har endret seg
        if status != status_forrige:
            autonom_status = status
            print(f"[AUTONOM] {status}")
            status_forrige = status

        if forsøk >= 5:
            print("[AUTONOM] Avbrutt – for mange blokkerte forsøk")
            autonom_aktiv = False
            send_to_arduino("MOV:X=0,Y=0,R=0")
            autonom_status = "Autonom stoppet – blokkert"
            break
